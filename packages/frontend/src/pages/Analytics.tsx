...
export const Analytics = () => {
  ...
  // Fetch real volume data from the backend to replace mock data
  const { data: volumeData } = useQuery({
    queryKey: ['volume'],
    queryFn: async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/analytics/volume?interval=hour`);
      const json = await res.json();
      return json.data?.volumeData || [];
    },
    refetchInterval: 60000
  });

  // Use real data if available, otherwise fallback to mock
  const displayData = volumeData?.length > 0 ? volumeData.map((d: any) => ({
    time: new Date(d.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    transactions: d.transactionCount,
    hashrate: 400 + Math.random() * 50, // Hashrate history not yet in API, keeping mock
    price: 30 + Math.random() * 2 // Price history not yet in API, keeping mock
  })) : mockHistoryData;
