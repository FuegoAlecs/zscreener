...
export const getStats = async () => {
  ...
  const [priceRes, blockRes, analyticsRes] = await Promise.allSettled([
    api.get('/price').catch(() => ({ data: { zec: { usd: 0, usd_24h_change: 0 } } })), // Fallback default
    api.get('/transactions?limit=1').catch(() => ({ data: { data: { transactions: [] } } })),
    api.get('/analytics/network-stats').catch(() => ({ data: { data: { networkHashrate: 0 } } }))
  ]);
